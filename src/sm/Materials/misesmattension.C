...
namespace oofem {
REGISTER_Material(MisesMatTension);

MisesMatTension :: MisesMatTension(int n, Domain *d) : StructuralMaterial(n, d),
    linearElasticMaterial(n, d)
{}


void
MisesMatTension :: initializeFrom(InputRecord &ir)
{
    ...
    IR_GIVE_FIELD(ir, sig0, _IFT_MisesMatTension_sig0); // uniaxial yield stress (scalar function)
    IR_GIVE_FIELD(ir, s0, _IFT_MisesMatTension_sig0); // uniaxial yield stress (double) used in giveQcPlasticParamneter()

    H = 0.;
    IR_GIVE_OPTIONAL_FIELD(ir, H, _IFT_MisesMatTension_h); // hardening modulus

    omega_crit = 0;
    IR_GIVE_OPTIONAL_FIELD(ir, omega_crit, _IFT_MisesMatTension_omega_crit); // critical damage

    a = 0;
    IR_GIVE_OPTIONAL_FIELD(ir, a, _IFT_MisesMatTension_a); // exponent in damage law

    yieldTol = 1.e-6;
    IR_GIVE_OPTIONAL_FIELD(ir, yieldTol, _IFT_MisesMatTension_yieldTol); // tolerance in the yield condition
}


// creates a new material status  corresponding to this class
MaterialStatus *
MisesMatTension :: CreateStatus(GaussPoint *gp) const
{
    return new MisesMatTensionStatus(gp);
}

FloatArrayF<1>
MisesMatTension :: giveRealStressVector_1d(const FloatArrayF<1> &totalStrain,
                                    GaussPoint *gp,
...
#if 1
    auto status = static_cast< MisesMatTensionStatus * >( this->giveStatus(gp) );
...
FloatArrayF<3>
MisesMatTension :: giveRealStressVector_PlaneStress(const FloatArrayF<3> &totalStrain,
                                             GaussPoint *gp, TimeStep *tStep) const
{
    auto status = static_cast< MisesMatTensionStatus * >( this->giveStatus(gp) );
    // initialization
    const_cast<MisesMatTension*>(this)->initTempStatus(gp);
...
FloatArrayF<6>
MisesMatTension :: giveRealStressVector_3d(const FloatArrayF<6> &strain, GaussPoint *gp,
                                    TimeStep *tStep) const
{
    auto status = static_cast< MisesMatTensionStatus * >( this->giveStatus(gp) );
    // subtract stress independent part
...
void
MisesMatTension :: performPlasticityReturn(const FloatArray &totalStrain, GaussPoint *gp, TimeStep *tStep) const
{
    auto status = static_cast< MisesMatTensionStatus * >( this->giveStatus(gp) );
    double kappa;
    FloatArray plStrain;
    ...
    if ( totalStrain.giveSize() == 1 ) {
        ...
        /*yield function*/
        //double yieldValue = trialS - ( this->give('s', gp, tStep) + H * kappa ); // original
    	double yieldStress = (this->give('s', gp, tStep) + H * kappa); // km sig0 = this->give('s', gp, tStep)
...
void
MisesMatTension :: performPlasticityReturn_PlaneStress(const FloatArrayF<3> &totalStrain, GaussPoint *gp, TimeStep *tStep) const
{
    double E = linearElasticMaterial.give('E', gp);
    double nu = linearElasticMaterial.give('n', gp);
    MisesMatTensionStatus *status = static_cast< MisesMatTensionStatus * >( this->giveStatus(gp) );
    double kappa;
    ...
    if ( yieldValue / sigmaY > yieldTol ) {
        ...
        while ( true ) {
            ...
            iter++;
            if ( iter > 400 ) {
                OOFEM_WARNING("No convergence of the stress return algorithm in MisesMatTension :: performPlasticityReturn_PlaneStress\n");
                break;
            }
        }
...
double
MisesMatTension :: computeYieldStress(double kappa, GaussPoint *gp, TimeStep *tStep) const
{
    return this->give('s', gp, tStep) + this->H * kappa; // + ( this->sigInf - this->sig0 ) * (1. - exp(-expD*kappa));
}

double
MisesMatTension :: computeYieldStressPrime(double kappa) const
{
...
double
MisesMatTension :: computeDamageParam(double tempKappa) const
{
    if ( tempKappa > 0. ) {
...
double
MisesMatTension :: computeDamageParamPrime(double tempKappa) const
{
    if ( tempKappa >= 0. ) {
...
double
MisesMatTension :: computeDamage(GaussPoint *gp,  TimeStep *tStep) const
{
    auto status = static_cast< MisesMatTensionStatus * >( this->giveStatus(gp) );
...
double MisesMatTension :: computeCumPlastStrain(GaussPoint *gp, TimeStep *tStep) const
{
    auto status = static_cast< MisesMatTensionStatus * >( this->giveStatus(gp) );
...
FloatMatrixF<6,6>
MisesMatTension :: give3dMaterialStiffnessMatrix(MatResponseMode mode,
                                          GaussPoint *gp,
                                          TimeStep *tStep) const
{
...
FloatMatrixF<3,3>
MisesMatTension :: givePlaneStressStiffMtrx(MatResponseMode mmode, GaussPoint *gp, TimeStep *tStep) const
{
    auto status = static_cast< MisesMatTensionStatus * >( this->giveStatus(gp) );
    // start from the elastic stiffness
...
FloatMatrixF<1,1>
MisesMatTension :: give1dStressStiffMtrx(MatResponseMode mode,
                                  GaussPoint *gp,
                                  TimeStep *tStep) const
{
    MisesMatTensionStatus *status = static_cast< MisesMatTensionStatus * >( this->giveStatus(gp) );
    ...
    // zero stress if linear softening gives negative stress
    double eps = status->giveStrainVector()[0];
    if  ( (H<0) & (eps >= this->give('s', gp, tStep)/E - this->give('s', gp, tStep)*(E + H)/(E*H) ) ) { 
      return {E/1000000};
    }
...
int
MisesMatTension :: giveIPValue(FloatArray &answer, GaussPoint *gp, InternalStateType type, TimeStep *tStep)
{
    MisesMatTensionStatus *status = static_cast< MisesMatTensionStatus * >( this->giveStatus(gp) );
...
MisesMatTensionStatus :: MisesMatTensionStatus(GaussPoint *g) :
...
void
MisesMatTensionStatus :: printOutputAt(FILE *file, TimeStep *tStep) const
{
...
// initializes temporary variables from given data
void MisesMatTensionStatus :: initTempStatusFromData(const FloatArray &statusData)
{
...
void MisesMatTensionStatus :: initTempStatus()
{
    StructuralMaterialStatus :: initTempStatus();
...
void
MisesMatTensionStatus :: updateYourself(TimeStep *tStep)
{
...
double
MisesMatTension :: give(int aProperty, GaussPoint *gp, TimeStep *tStep) const
{
    if ( aProperty == 's' ) {
...
double MisesMatTension :: giveTemperature(GaussPoint *gp, TimeStep *tStep) const
{
...
void
MisesMatTensionStatus :: saveContext(DataStream &stream, ContextMode mode)
{
...
void
MisesMatTensionStatus :: restoreContext(DataStream &stream, ContextMode mode)
{
